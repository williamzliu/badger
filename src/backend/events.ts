...
export class EventLog {
  ...
  append(sessionId:string,type:string,publicMessage:string,privateData:Record<string,unknown>={}):BadgerEvent {const event={id:randomUUID(),sessionId,type,timestamp:new Date().toISOString(),publicMessage,privateData};this.db.prepare('INSERT INTO events VALUES (?, ?, ?, ?, ?, ?)').run(event.id,event.sessionId,event.type,event.timestamp,event.publicMessage,JSON.stringify(event.privateData));this.subscribers.get(sessionId)?.forEach(subscriber=>subscriber(event));return event;}
